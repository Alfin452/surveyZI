...
<div class="space-y-6">
    <div class="bg-white rounded-lg p-5 border border-gray-200 shadow-sm">
        <div class="flex flex-col md:flex-row md:justify-between md:items-start">
            ...
            @if($selectedProgram)
            <a href="{{ route('superadmin.reports.export', ['program_id' => $selectedProgram->id]) }}" class="mt-4 md:mt-0 bg-green-600 text-white px-5 py-2 rounded-lg font-medium hover:bg-green-700 transition duration-300 shadow-sm flex items-center space-x-2 self-start md:self-end">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-indigo-50">
                <tr>
                    ...
                    @php $section = $section->title; @endphp
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($reportData as $unitReport)
                <tr class="hover:bg-indigo-50/50 group">
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm sticky left-0 group-hover:bg-indigo-50/50">
                        <p class="text-gray-900 font-semibold whitespace-no-wrap">{{ $unitReport['unit_name'] }}</p>
                    </td>
                    ...
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm font-bold text-center sticky right-0 group-hover:bg-indigo-50/50">
                        <p class="text-indigo-700 whitespace-no-wrap">{{ number_format($unitReport['total_avg'], 2) }}</p>
                    </td>
                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm text-center group-hover:bg-indigo-50/50">
                        <a href="{{ route('superadmin.reports.showUnit', [$selectedProgram->id, $unitReport['unit_id']]) }}" class="text-blue-600 hover:text-blue-800 font-semibold">
                            Detail
                        </a>
                    </td>
                </tr>
