...
@section('content')
<div class="space-y-1" x-data="sectionManager(@json($program->questionSections->pluck('id')))"
    x-init="init()"
            ...
            <div>
                <nav class="text-sm mb-2 font-medium text-gray-500" aria-label="Breadcrumb">
                    <ol class="list-none p-0 inline-flex">
                        <li class="flex items-center"><a href="{{ route('superadmin.dashboard') }}" class="hover:text-gray-700">Dashboard</a><span class="mx-2">/</span></li>
                        <li class="flex items-center"><a href="{{ route('superadmin.programs.index') }}" class="hover:text-gray-700">Program Survei</a><span class="mx-2">/</span></li>
    ...
    <div x-ref="sectionContainer" class="space-y-6">
        ...
        <div data-id="{{ $section->id }}" class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="flex items-center justify-between p-4 bg-gray-50 border-b rounded-t-lg">
                ...
                <div class="flex items-center space-x-2">
                    <button
                        @click="$dispatch('open-edit-section-modal', @js($section->only(['id','title','description'])))"
                    ...
                    <button @click="$dispatch('open-delete-modal', { url: '{{ route('superadmin.sections.destroy', $section) }}', name: '{{ addslashes($section->title) }}' })" class="text-red-600 hover:text-red-800" title="Hapus Bagian">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <div class="p-4 space-y-2" x-data="questionSubManager(@json($section->questions->pluck('id')))" x-init="initQuestionSortable('{{ route('superadmin.sections.questions.reorder', $section) }}')">
                <div class="space-y-3" x-ref="questionContainer">
                    @forelse ($section->questions as $question)
                    <div data-id="{{ $question->id }}" x-data="{ isOptionsVisible: false }" class="list-none bg-gray-50 border rounded-lg p-3 group">
                        <div class="flex items-start justify-between">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div @click="isOptionsVisible = !isOptionsVisible" class="flex items-start justify-between cursor-pointer group/trigger pr-2">
                                        <div class="flex items-start flex-1">
                                            <span class="item-number font-semibold text-gray-800 mr-2 pt-0.5">{{ $loop->iteration }}.</span>
                                            <p class="font-semibold text-gray-800 flex-1 pt-0.5">{{ $question->question_body }}</p>
                                        </div>
                                    ...
                                    <div x-cloak x-show="isOptionsVisible" x-transition class="flex flex-wrap gap-2 mt-3 pt-3 pl-6 border-t border-gray-200">
                                        ...
                                        <span class="inline-flex items-center text-xs font-medium bg-gray-100 text-gray-700 px-2 py-0.5 rounded-md">
                                            {{ $option->option_body }}
                                            <span class="font-bold text-gray-900 ml-1.5">({{ $option->option_score }})</span>
                                        </span>
                                        @empty
                            ...
                            <div class="flex items-start space-x-2 opacity-0 group-hover:opacity-100 transition-opacity ml-4 pt-1">
                                <a href="{{ route('superadmin.sections.questions.edit', ['section' => $section, 'question' => $question]) }}" class="text-blue-600 hover:text-blue-800" title="Edit Pertanyaan">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <button @click="$dispatch('open-delete-modal', { url: '{{ route('superadmin.sections.questions.destroy', ['section' => $section, 'question' => $question]) }}', name: '{{ addslashes($question->question_body) }}' })" class="text-red-600 hover:text-red-800" title="Hapus Pertanyaan">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6M9 7V4a1 1 0 011-1h4a1 1 0 011 1v3M4 7h16" />
                                    </svg>
                                </button>
                ...
                <div class="flex items-center justify-between pt-3 border-t mt-4">
                    ...
                    <a href="{{ route('superadmin.sections.questions.create', $section) }}" class="inline-flex items-center px-3 py-1 border border-dashed border-green-400 text-sm font-medium rounded-md text-green-700 bg-green-50 hover:bg-green-100 ml-auto">
    ...
    <div x-cloak x-show="showAddModal" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showAddModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block w-full max-w-lg p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                ...
                <form action="{{ route('superadmin.programs.sections.store', $program) }}" method="POST" class="mt-4 space-y-4">
                    @csrf
                    <div>
