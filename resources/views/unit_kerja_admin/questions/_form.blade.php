...
<div x-data='{
    ...
    <div class="space-y-6 bg-white p-6 rounded-xl shadow-lg border">
        ...
        <div>
            ...
            <textarea name="question_body" id="question_body" rows="3" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500 sm:text-sm">{{ old('question_body', $question->question_body ?? '') }}</textarea>
    ...
    <div class="mt-8 pt-6 border-t flex justify-end space-x-3">
        <a href="{{ route('unitkerja.admin.surveys.show', $survey) }}" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50">
            Batal
        </a>
