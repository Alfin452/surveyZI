...
<div class="space-y-6" x-data="{ activeTab: 'institusional' }">
    ...
    <div class="bg-white rounded-lg p-5 border border-gray-200 shadow-sm">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center">
            ...
            <a href="{{ route('unitkerja.admin.my-programs.create') }}" class="mt-4 md:mt-0 bg-green-600 text-white px-5 py-2 rounded-lg font-medium hover:bg-green-700 transition duration-300 shadow-sm flex items-center space-x-2 self-start md:self-end">
        ...
        <div class="border-b border-gray-200">
            <nav class="-mb-px flex space-x-6" aria-label="Tabs">
                <button @click="activeTab = 'institusional'"
                    ...
                    <span class="ml-1.5 rounded-full {{ $institutionalPrograms->count() > 0 ? 'bg-gray-200 text-gray-700' : 'bg-gray-100 text-gray-500' }} py-0.5 px-2 text-xs">{{ $institutionalPrograms->count() }}</span>
                </button>
                <button @click="activeTab = 'myPrograms'"
                    ...
                    class="whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200">
                    Program Unit Saya
                    <span class="ml-1.5 rounded-full {{ $myPrograms->total() > 0 ? 'bg-gray-200 text-gray-700' : 'bg-gray-100 text-gray-500' }} py-0.5 px-2 text-xs">{{ $myPrograms->total() }}</span>
                </button>
            </nav>
        </div>
    </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-center align-middle">
                                <a href="{{ route('unitkerja.admin.programs.results', $program) }}" class="inline-block bg-gray-800 text-white px-4 py-2 rounded-lg font-semibold hover:bg-gray-700 transition shadow-sm">
                                    Lihat Hasil
                                </a>
                            </td>
                        </tr>
            ...
            <div class="overflow-x-auto bg-white rounded-lg shadow-sm border border-gray-200">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 align-middle">
                                <a href="{{ route('unitkerja.admin.programs.questions.index', $program) }}" class="font-semibold text-gray-800 hover:text-black">{{ $program->title }}</a>
                                <div class="text-sm text-gray-500">{{ Str::limit($program->description, 80) }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium align-middle">
                                <div class="flex items-center justify-center space-x-3">
                                    ...
                                    <a href="{{ route('unitkerja.admin.my-programs.edit', $program) }}" class="text-blue-600 hover:text-blue-800" title="Edit Program">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
