    ...
    <div class="mb-8 bg-white rounded-xl p-4 md:p-6 border-l-4 border-teal-500 shadow-sm">
        <div class="flex flex-col md:flex-row md:justify-between md:items-start">
            ...
            <div class="mt-4 md:mt-0 flex space-x-2 self-start md:self-end">
                ...
                {{-- PERBAIKAN: Tautan sekarang berfungsi --}}
                <a href="{{ route('unitkerja.admin.surveys.questions.create', $survey) }}" class="bg-teal-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-teal-700 transition">Tambah Pertanyaan</a>
            </div>
        </div>
    </div>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-gray-50 border rounded-lg p-4 flex items-center justify-between group">
                ...
                <div class="flex items-center space-x-3 opacity-0 group-hover:opacity-100 transition-opacity">
                    {{-- PERBAIKAN: Tautan Edit & Hapus sekarang berfungsi --}}
                    <a href="{{ route('unitkerja.admin.surveys.questions.edit', ['survey' => $survey, 'question' => $question]) }}" class="text-indigo-600 hover:text-indigo-800" title="Edit Pertanyaan">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    </a>
                    <button type="button" @click="$dispatch('open-delete-modal', { url: '{{ route('unitkerja.admin.surveys.questions.destroy', ['survey' => $survey, 'question' => $question]) }}', name: '{{ addslashes($question->question_body) }}' })" class="text-red-600 hover:text-red-800" title="Hapus Pertanyaan">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
