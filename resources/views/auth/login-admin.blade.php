...
<body class="bg-slate-900 h-screen overflow-hidden flex items-center justify-center relative">
    ...
    {{-- Background Aurora Gelap (Kesan Aman & Misterius) --}}
    ...
    <div class="relative z-10 w-full max-w-md p-4">
        <div class="bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl rounded-3xl p-8 relative overflow-hidden">
            ...
            {{-- Form Login --}}
            <form method="POST" action="{{ url('portal-admin/masuk') }}" class="space-y-5">
                @csrf
                ...
                <div>
                    ...
                    @error('username') <span class="text-rose-400 text-xs mt-1 ml-1">{{ $message }}</span> @enderror
                </div>
            ...
            <div class="mt-8 text-center">
                <a href="{{ route('home') }}" class="text-xs text-white/40 hover:text-white transition-colors">
