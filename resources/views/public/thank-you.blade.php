<x-guest-layout title="Terima Kasih">
    ...
    <div class="min-h-screen flex items-center justify-center p-4 relative z-10 pt-32 pb-20"
        ...
        <div class="w-full max-w-lg bg-white/90 backdrop-blur-xl rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-white/60 p-10 sm:p-14 text-center form-anim relative overflow-hidden">
            ...
            <div class="space-y-3">
                <a href="{{ route('home') }}" class="block w-full py-4 bg-slate-900 text-white rounded-2xl font-bold text-sm uppercase tracking-widest shadow-lg shadow-slate-300/50 hover:bg-emerald-600 transition-all hover:-translate-y-0.5">
                    Kembali ke Beranda
                </a>
                @auth
                <a href="{{ route('public.profile.history') }}" class="block w-full py-4 bg-white border border-slate-200 text-slate-600 rounded-2xl font-bold text-sm uppercase tracking-widest hover:bg-slate-50 transition-all">
                    Lihat Riwayat Saya
                </a>
                @endauth
            </div>

            {{-- FITUR 3: AUTO REDIRECT MESSAGE --}}
            <div class="mt-10 pt-6 border-t border-slate-100">
                <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">
                    © {{ date('Y') }} SurveyZI UIN Antasari
                </p>
