...
<div class="relative z-10 space-y-8"
    x-data="sectionManager(<?php echo json_encode($program->questionSections->pluck('id'), 15, 512) ?>)"
    x-init="init()"
            ...
            <div class="flex items-start gap-4">
                ...
                <div>
                    <nav class="flex items-center text-xs font-medium text-slate-500 mb-1 space-x-2">
                        <a href="<?php echo e(route('unitkerja.admin.programs.index')); ?>" class="hover:text-teal-600 transition-colors">Program</a>
                    ...
                    <p class="text-slate-500 text-sm font-medium mt-0.5 line-clamp-1">
                        Program: <span class="text-teal-600 font-bold"><?php echo e($program->title); ?></span>
                    </p>
                </div>
            </div>

            <div class="flex flex-wrap items-center gap-3">
                <a href="<?php echo e(route('unitkerja.admin.programs.index')); ?>" class="px-4 py-2 bg-white border border-slate-200 text-slate-600 rounded-xl text-xs font-bold hover:bg-slate-50 transition shadow-sm">
                    Kembali
                </a>
                <button type="button" x-show="isSectionOrderChanged" x-transition
                    @click="saveSectionOrder('<?php echo e(route('unitkerja.admin.programs.sections.reorder', $program)); ?>')"
    ...
    <div x-ref="sectionContainer" class="space-y-6 pb-10">
        ...
        <div data-id="<?php echo e($section->id); ?>" class="bg-white/80 backdrop-blur-md rounded-3xl border border-white/50 shadow-sm hover:shadow-md transition-all duration-300 group/section">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-slate-800 flex items-center gap-2">
                            ...
                            <span class="text-[10px] bg-teal-100 text-teal-600 px-2 py-0.5 rounded-full font-extrabold tracking-wide">
                                <?php echo e($section->questions_count); ?> SOAL
                            </span>
                        </h2>
                        <?php if($section->description): ?> <p class="text-xs text-slate-500 mt-0.5"><?php echo e($section->description); ?></p> <?php endif; ?>
                    </div>
                </div>

                <div class="flex items-center gap-1 opacity-60 group-hover/section:opacity-100 transition-opacity">
                    <button @click="$dispatch('open-edit-section-modal', <?php echo \Illuminate\Support\Js::from($section->only(['id','title','description']))->toHtml() ?>)"
                    ...
                    <button @click="$dispatch('open-delete-modal', { url: '<?php echo e(route('unitkerja.admin.sections.destroy', $section)); ?>', name: '<?php echo e(addslashes($section->title)); ?>' })"
            ...
            <div class="p-5 bg-white/40 rounded-b-3xl" x-data="questionSubManager(<?php echo json_encode($section->questions->pluck('id'), 15, 512) ?>)" x-init="initQuestionSortable('<?php echo e(route('unitkerja.admin.sections.questions.reorder', $section)); ?>')">
                <div class="space-y-3 min-h-[50px]" x-ref="questionContainer">
                    ...
                    <div data-id="<?php echo e($question->id); ?>" x-data="{ isOptionsVisible: false }"
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <div @click="isOptionsVisible = !isOptionsVisible" class="flex justify-between cursor-pointer select-none">
                                    <div class="flex gap-3">
                                        <span class="item-number flex-shrink-0 w-6 h-6 rounded-full bg-slate-100 text-slate-600 text-xs font-bold flex items-center justify-center mt-0.5"><?php echo e($loop->iteration); ?></span>
                                        <p class="text-sm font-semibold text-slate-700 leading-relaxed group-hover:text-teal-700 transition-colors"><?php echo e($question->question_body); ?></p>
                                    </div>
                                ...
                                <div x-show="isOptionsVisible" x-collapse class="mt-4 pl-9">
                                    <div class="flex flex-wrap gap-2">
                                        ...
                                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[11px] font-medium bg-slate-50 text-slate-600 border border-slate-200">
                                            ...
                                            <span class="bg-teal-100 text-teal-600 px-1.5 rounded text-[10px] font-bold ml-1"><?php echo e($option->option_score); ?></span>
                                        </span>
                            ...
                            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity pt-0.5">
                                <a href="<?php echo e(route('unitkerja.admin.sections.questions.edit', ['section' => $section, 'question' => $question])); ?>"
                                    class="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                    </svg>
                                </a>
                                <button @click="$dispatch('open-delete-modal', { url: '<?php echo e(route('unitkerja.admin.sections.questions.destroy', ['section' => $section, 'question' => $question])); ?>', name: '<?php echo e(addslashes($question->question_body)); ?>' })"
                ...
                <div class="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
                    ...
                    <a href="<?php echo e(route('unitkerja.admin.sections.questions.create', $section)); ?>"
    ...
    <div x-cloak x-show="showAddModal" class="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 text-center">
            ...
            <div x-show="showAddModal" class="inline-block w-full max-w-lg p-8 my-8 text-left align-middle transition-all transform bg-white rounded-3xl shadow-2xl relative z-10">
                ...
                <form action="<?php echo e(route('unitkerja.admin.programs.sections.store', $program)); ?>" method="POST" class="space-y-5">
                    <?php echo csrf_field(); ?>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.unit_kerja_admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\surveyZI\resources\views/unit_kerja_admin/programs/questions/index.blade.php ENDPATH**/ ?>